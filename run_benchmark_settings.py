...
for b in benchmarks:
        for t in threads:
                    for f in frequencies:
                                    command = "scale "+str(f)
                                    os.system(command)
                                    command = "sudo sh -c \"export OMP_NUM_THREADS="+t+";task_rapl -i -e cycles,CYCLE_ACTIVITY:STALLS_TOTAL,LLC_MISSES,CYCLE_ACTIVITY:STALLS_L3_MISS "+b+"> testresult"+str(frequencies.index(f))+".txt\""
                                    os.system(command)
