...
for b in benchmarks:
        for t in threads:
                    for f in frequencies:
                                    command = "scale "+str(f)
                                    os.system(command)
                                    command = "sudo sh -c \"export OMP_NUM_THREADS="+t+";task_rapl -i -e INST_RETIRED:ALL,MEM_LOAD_UOPS_RETIRED:L3_MISS,UOPS_RETIRED:ALL,OFFCORE_RESPONSE_1:L3_HITS "+b+" > testresult"+str(frequencies.index(f))+".txt\""
                                    os.system(command)
